...
class IBAInterpreter:
    def __init__(self, model, estim: Estimator, device, beta, steps=10, lr=1, batch_size=1, ensemble=False, progbar=False):
    ...
    def _train_bottleneck(self, text_t: torch.Tensor, image_t: torch.Tensor):
        ...
        if(not self.ensemble):
            ...
            # Train
            self.model.eval()
            for i in tqdm(range(self.train_steps), desc="Training Bottleneck",
                ...
                current_beta = beta_controller.update(loss_t)
                loss_t.backward()
...
class AdaptiveBetaController:
    def __init__(self, init_beta=1.0, max_beta=10.0, min_beta=0.1, factor=1.2, patience=3):
        self.beta = init_beta
        self.max = max_beta
        self.min = min_beta
        self.factor = factor
    ...
    def update(self, current_loss):
        if current_loss < self.best_loss:
            ...
            if self.no_improve >= self.patience:
                self.beta = min(self.beta * self.factor, self.max)
                self.no_improve = 0
        return self.beta
