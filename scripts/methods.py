...
def extract_feature_map(model, layer_idx, x, device):
    with torch.no_grad():
        states = model(x, output_attentions=True, output_hidden_states=True)
        feature = states['hidden_states'][layer_idx+1] # +1 because the first output is embedding
        attn_weight_list = states['attentions']
        attn_weight_stack = torch.stack(attn_weight_list, dim=0).permute(1, 2, 0, 3, 4) #[12,12,197,197]
        return feature, attn_weight_stack.squeeze(0)
...
def vision_heatmap_iba(text_t, image_t, model, layer_idx, beta, var, device, lr=1, train_steps=10,ensemble=False, progbar=True):
    features, attn_weight_list = extract_feature_map(model.vision_model, layer_idx, image_t, device) # [1,257,1024]
